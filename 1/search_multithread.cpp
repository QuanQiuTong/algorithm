...
int calculate_value(std::string_view ref, std::string_view query, std::string_view rcq, int ref_st, int ref_en, int query_st, int query_en)
{
    ...
    std::string_view b_rc = rcq.substr(query.size() - query_en, query_en - query_st);
...
void optimize_chunk(const std::string &ref, const std::string &query, const std::string &rcq, int ref_start_min, int ref_start_max, int step, Params &best_params, int &best_value)
{
    for (int ref_st = ref_start_min; ref_st < ref_start_max; ref_st += step)
    {
        for (int ref_en = 22500; ref_en < 25000; ref_en += step)
        {
            for (int query_st = 5500; query_st < 7200; query_st += step)
            {
                for (int query_en = 22500; query_en < 25000; query_en += step)
                {
                    int value = calculate_value(ref, query, rcq, ref_st, ref_en, query_st, query_en);
...
Params optimize_parameters(const std::string &ref, const std::string &query, const std::string &rcq)
{
    ...
    int ref_start_min = 5500, ref_start_max = 7200;
    ...
    for (int i = 0; i < num_threads; ++i)
    {
        ...
        threads.emplace_back(optimize_chunk, std::ref(ref), std::ref(query), std::ref(rcq), start, end, step, std::ref(best_params), std::ref(best_value));
    }
...
int main()
{
    ...
    std::string rcq = rc(query);
    strcpy(rcs, rcq.c_str());
    // cout << "lengths: " << ref.size() << " " << query.size() << " " << rcq.size() << endl;

    Params best_params = optimize_parameters(ref, query, rcq);
    int best_value = calculate_value(ref, query, rcq, best_params.ref_st, best_params.ref_en, best_params.query_st, best_params.query_en);
