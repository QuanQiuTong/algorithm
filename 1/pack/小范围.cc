...
static int calculate_value(int ref_st, int ref_en, int query_st, int query_en) {
...
#pragma omp parallel for collapse(4) schedule(dynamic) num_threads(20)
    for (int ref_st = ref_start_min; ref_st < ref_start_max; ref_st += step) {
        for (int ref_en = Sb; ref_en < Se; ref_en += step) {
            for (int query_st = Rb; query_st < Re; query_st += step) {
                for (int query_en = Sb; query_en < Se; query_en += step) {
                    int value = calculate_value(ref_st, ref_en, query_st, query_en);

                    if (value >= 29805)
                        fprintf(logfile, "%d %d %d %d %d\n", ref_st, ref_en, query_st, query_en, value);
                ...
                // fprintf(logfile, "%d %d %d  %d\n", ref_st, ref_en, query_st, local_value);
            }
            // fprintf(logfile, "%d %d  %d\n", ref_st, ref_en, local_value);
        }
    }
    {
