...
int calculate_value(int ref_st, int ref_en, int query_st, int query_en)
{
    ...
    EdlibAlignResult resultA = edlibAlign(rr, ref_st, ss, query_st, cfg);
    EdlibAlignResult resultB = edlibAlign(rr + ref_st, ref_en - ref_st, rcs + SLEN - query_en, query_en - query_st, cfg);
    EdlibAlignResult resultC = edlibAlign(rr + ref_en, RLEN - ref_en, ss + query_en, SLEN - query_en, cfg);
...
#pragma omp parallel for collapse(4) schedule(dynamic) num_threads(20)
    ...
    {
        for (int ref_en = 22800; ref_en < 24000; ref_en += step)
        {
            for (int query_st = 6100; query_st < 7200; query_st += step)
            {
                for (int query_en = 22800; query_en < 24000; query_en += step)
                {
...
Params optimize_parameters()
{
    ...
    int ref_start_min = 6100, ref_start_max = 7200;
...
int main()
{
    ...
    int best_value = calculate_value(ref_st, ref_en, query_st, query_en);
